...
function DetailPanel({ item, onClose, onReload }) {
  ...
  // modal: null | 'confirm-approve' | 'reject' | { type: 'approve-result', data }
...
export default function ChannelScreeningListPage() {
  ...
  const [filters, setFilters] = useState({
    startDate: '', endDate: '', channelType: '', status: '',
  });
  ...
  const selectedItem =
    allItems.find((i) => i.screeningNo === selectedNo) ?? null;
              ...
              <tbody>
                ...
                {!loading && items.map((item) => (
                  <tr
                    ...
                    onClick={() =>
                      setSelectedNo((p) => p === item.screeningNo ? null : item.screeningNo)
                    }
                  >
